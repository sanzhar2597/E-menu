...
public class TelegramStart extends TelegramLongPollingBot {

    public static void main(String[] args) {
        ApiContextInitializer.init();
        TelegramBotsApi telegramBotsApi = new TelegramBotsApi();
        try {
            telegramBotsApi.registerBot(new TelegramStart());
    ...
    public void onUpdateReceived(Update update) {
        ...
        if (message != null && message.hasText()) {
            switch (message.getText()) {
                ...
                case "/pushkin":
                    sendMsg(message, menu.getNameFromDb());
                    break;
                default:
                    try {
                        sendMsg(message, Menu.getWeather(message.getText(), model));
                    } catch (IOException e) {
